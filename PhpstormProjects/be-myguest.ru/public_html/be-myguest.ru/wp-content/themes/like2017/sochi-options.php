...
function phonemail_options() {
	...
	<table class="form-table table table-striped">
		<tr valign="top">
			<th scope="row">ВК</th>
			<td><input type="text" name="vk" size="60" value="<?php echo esc_attr( get_option('vk') ); ?>" /></td>
		</tr>

		<tr valign="top">
			<th scope="row">Фэйсбук</th>
			<td><input type="text" name="facebook" size="60" value="<?php echo esc_attr( get_option('facebook') ); ?>" /></td>
		</tr>

		<tr valign="top">
			<th scope="row">Инстаграм</th>
			<td><input type="text" size="60" name="instagram" value="<?php echo esc_attr( get_option('instagram') ); ?>" /></td>
		</tr>
	</table>
	<?submit_button();
	echo '</form>';
...
function register_mysettings() { // whitelist options
	register_setting( 'myoptiongroup', 'vk' );
	register_setting( 'myoptiongroup', 'facebook' );
	register_setting( 'myoptiongroup', 'instagram' );
}