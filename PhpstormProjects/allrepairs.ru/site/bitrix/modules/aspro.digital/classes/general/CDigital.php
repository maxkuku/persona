...
class CDigital{
	...
	static function ShowLogo(){
		global $arSite;
		$arTheme = self::GetFrontParametrsValues('s1');
	...
	function GetBannerStyle($bannerwidth, $topmenu){
        ...
        elseif($bannerwidth == "NARROW"){
            ...
            $style = ".maxwidth-banner{max-width: 100%;}";
        }

        return "<style>".$style."</style>";
    }

    function GetIndexPageBlocks($pageAbsPath, $pageBlocksPrefix, $pageBlocksDirName = 'page_blocks'){
    	$arResult = array();
    ...
    function GetComponentTemplatePageBlocks($templateAbsPath, $pageBlocksDirName = 'page_blocks'){
    ...
    function GetComponentTemplatePageBlocksParams($arPageBlocks){
    	$arResult = array();
    ...
    protected function IsComponentTemplateHasModuleElementsPageBlocksParam($templateName){
	    return $templateName && isset(self::$arParametrsList['SECTION']['OPTIONS'][strtoupper($templateName).'_PAGE']);
    }

    protected function IsComponentTemplateHasModuleElementPageBlocksParam($templateName){
	    return $templateName && isset(self::$arParametrsList['SECTION']['OPTIONS'][strtoupper($templateName).'_PAGE_DETAIL']);
    }

    function AddComponentTemplateModulePageBlocksParams($templateAbsPath, &$arParams){
    	if($templateAbsPath && $arParams && is_array($arParams)){
    		$templateAbsPath = str_replace('//', '//', $templateAbsPath).'/';
    		$templateName = basename($templateAbsPath);
    		if(self::IsComponentTemplateHasModuleElementsPageBlocksParam($templateName)){
    		...
    		if(self::IsComponentTemplateHasModuleElementPageBlocksParam($templateName)){
    ...
    function CheckComponentTemplatePageBlocksParams(&$arParams, $templateAbsPath, $pageBlocksDirName = 'page_blocks'){
    ...
    function Add2OptionCustomComponentTemplatePageBlocks(&$arOption, $templateAbsPath){
