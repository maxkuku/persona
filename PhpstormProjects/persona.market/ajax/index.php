...
# personal consult form
if (htmlspecialchars($_REQUEST['personal'], 3) == "Y" && strlen(htmlspecialchars($_REQUEST['phone'], 3)) > 0) {
    ...
    if ($g == 1) {
        if ($RESULT_ID = CFormResult::Add($WEB_FORM_ID, $arValues)) {
            ...
            echo "<button type=\"button\" class=\"close\" onclick=\"set_cook('popup', 'Y', 3)\" data-dismiss=\"modal\">×</button><div class=\"mod-answer\"><p>Заявка принята, спасибо. Номер заявки " . $RESULT_ID . "</p></div>";
        }
    } else {
        echo "<button type=\"button\" class=\"close\" data-dismiss=\"modal\">×</button><div class=\"mod-answer\"><p>Неправильно введен номер телефона</p></div>";
    }
}
...
if ($PRODUCT_ID && $WEB_FORM_ID) {
    $quan = htmlspecialchars($_REQUEST['quantity'], 3);
    ...
    if ($r > 0) {
        ...
        $json['good'] = "Товар <a href=\"" . $P_URL . "\">" . $PROD_NAME . " добавлен в вашу <a href=\"/basket/\">корзину</a>! " . $quantity . "шт. на сумму " . $PRICE * $quantity . " р.";
...
# append basket in list https://persona.market/basket/list/
$PRODUCT_ID = (int)htmlspecialchars($_REQUEST['ind'], 3);
$UPD = (int)htmlspecialchars($_REQUEST['kol'], 3);
...
$PRODUCT_ID = (int)htmlspecialchars($_REQUEST['key'], 3);
                ...
                <form action="" method="post" enctype="multipart/form-data" id="callback_form">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input type="text" name="form_text_20" value="" id="input-name" class="form-control"
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input type="text" name="form_text_21" value="" id="input-phone" class="form-control"
                                   placeholder="Ваш телефон">
                        </div>
                    </div>
                    <input type="hidden" name="callback_order" value="Y"/>
                    <input type="hidden" name="sessid" value="<?= substr(bitrix_sessid_get(), 7) ?>"/>
                    <button class="btn btn-danger btn-block" name="web_form_submit" type="button"
                            onclick="callback_send()"><i class="fa fa-bolt fa-fw"></i>&nbsp;Заказать
                    </button>
                </form>
            </div>
        </div>
    ...
    if ((int)htmlspecialchars($_REQUEST['product_id'], 3) > 0) {
        ...
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"
                    onclick="cart.remove('<?= $arFields['ID'] ?>');return false;">×
            </button>
            ...
            <div class="row">
                <div class="col-sm-6 text-center">
                    <div class="image">
                        <?
                        if ($rsFile): ?>
                            <img src="<?= $rsFile ?>" onclick="location.href='<?= $arFields['DETAIL_PAGE_URL'] ?>'"
                                 alt="<?= $arFields['NAME'] ?>" class="img-responsive center-block">
                        <? else: ?>
                            <img src="/bitrix/templates/persona/images/no-photo.png"
                                 onclick="location.href='<?= $arFields['DETAIL_PAGE_URL'] ?>'" alt="No image"
                                 class="img-responsive center-block">
                        <?endif ?>
                    </div>
                    <div class="price"><?= (!$arFields['PROPERTY_SALE_VALUE']) ? $arFields['PROPERTY_PRICE_VALUE'] . " <span class=\"roboto-forced ruble price-real\" aria-hidden=\"true\" style=\"display:none;\"></span>" : "" ?>
                        ...
                        <?
                        if ($arFields['PROPERTY_SALE_VALUE']): ?>
                            <span class="price-old">&nbsp;<?= ($arFields['PROPERTY_SALE_VALUE']) ? $arFields['PROPERTY_PRICE_VALUE'] : "" ?> <span
                                        class="sr-only">р.</span><span class="roboto-forced ruble" aria-hidden="true"
                                                                       style="display:none;"></span>&nbsp;</span>
                            <b><?= $arFields['PROPERTY_SALE_VALUE'] ?> <span class="sr-only">р.</span><span
                                        class="roboto-forced ruble" aria-hidden="true" style="display:none;"></span></b>
                        <?endif ?>
                    </div>
                </div>
                <div class="col-sm-6">
                    <form action="/ajax/" method="post" enctype="multipart/form-data" id="fastorderform">
                        ...
                        <div class="form-group">
                            <div class="input-group ">
                                <span class="input-group-addon"><i class="fa fa-user fa-fw"></i></span>
                                <input type="text" name="form_text_13" value="" id="input-name" class="form-control"
                                       placeholder="Ваше имя">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="input-group">
                                <span class="input-group-addon"><i class="fa fa-phone fa-fw"></i></span>
                                <input type="text" name="form_text_12" value="" id="input-phone" class="form-control"
                                       placeholder="Ваш телефон">
                            </div>
                        </div>
                        <div class="form-group">
                            <textarea name="MESSAGE" rows="5" id="input-enquiry" class="form-control"
                                      placeholder="Ваш комментарий, если требуется"></textarea>
                        </div>
                        <?
                        ...
                        <?
                        $pr = (!$arFields['PROPERTY_SALE_VALUE']) ? $arFields['PROPERTY_PRICE_VALUE'] : $arFields['PROPERTY_SALE_VALUE'] ?>
                        <button class="btn btn-danger btn-block" name="web_form_submit" type="button" <?
                        if ($pr >= MIN_ORDER_PRICE): ?> onclick="fastordersend()" <? else: ?> disabled title="Сумма заказа менее минимальной"<?endif ?>>
                            <i class="fa fa-bolt fa-fw"></i>&nbsp;Заказать
                        </button>
                    </form>
                </div>
            </div>
