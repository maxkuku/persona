...
if(!function_exists("FillAllPicturesAndDescriptions"))
{
	function FillAllPicturesAndDescriptions(&$arAllItems, $arMenuItemsIDs)
	{
		...
		{
			if ($arItem["DEPTH_LEVEL"] == "1" || $arItem["PARAMS"]["DEPTH_LEVEL"] == "1")
			{
				if ($arItem["IS_PARENT"])
				{
			...
			elseif($arItem["DEPTH_LEVEL"] == "2" || $arItem["PARAMS"]["DEPTH_LEVEL"] == "2")
			{
			...
			elseif($arItem["DEPTH_LEVEL"] == "3" || $arItem["PARAMS"]["DEPTH_LEVEL"] == "3")
			{
				if (!$arItem["PARAMS"]["picture_src"])
...
foreach($arResult as $key=>$arItem)
{
	...
	$arItem["PARAMS"]['DEPTH_LEVEL'] = $arItem['DEPTH_LEVEL'];
	$arItem["PARAMS"]['IS_PARENT'] = $arItem['IS_PARENT'];
