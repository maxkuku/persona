...
class CatalogElementComponent extends Element
{
	...
	{
		$filter = array("IBLOCK_ID" => "5", "ACTIVE"=>"Y", "=PROPERTY_FOR_WHAT" => $id);
		$items_sel =  (new CIBlockElement())->GetList([], $filter, 0, 0, ['ID',
            ...
            'PROPERTY_USEFULL',
            'PROPERTY_FEATURES',
            'PROPERTY_HOW_IS_PRICE',
            'DETAIL_TEXT',
            'PROPERTY_YOUR_RATING',
            'PROPERTY_NO_FEATURES']
        );
		...
		if($arResult['ITEMS_SELECTED'] < 1) {
			...
			while ( $item = $items_sel->Fetch() ) {
                ...
                $arFeed[$item['ID']] = array(
                    ...
                    $item['DETAIL_TEXT'],
                    $item['TIMESTAMP_X'],
                    $item['PROPERTY_YOUR_RATING_VALUE'],
                    $item['PROPERTY_FOR_WHAT_VALUE'],
                    $item['PROPERTY_USEFULL_VALUE'],
                    $item['PROPERTY_FEATURES_VALUE'],
                    $item['PROPERTY_HOW_IS_PRICE_VALUE'],
                    $item['PROPERTY_NO_FEATURES_VALUE']);

				# подсчет рейтинга и пр.

				if($item['PROPERTY_YOUR_RATING_VALUE'] > $maximum)
					$maximum = $item['PROPERTY_YOUR_RATING_VALUE'];

				if($item['PROPERTY_YOUR_RATING_VALUE'] > 0 && $item['PROPERTY_YOUR_RATING_VALUE'] < $minimum)
					$minimum = $item['PROPERTY_YOUR_RATING_VALUE'];
			}

			if($feed && $items)
			    $rate = round( $feed / $items );
		}
		$feeds = array($rate,$items,$maximum,$minimum,$err,$arFeed);
