...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
    ...
    $res = $DB->Query("SELECT USER_TEXT FROM b_form_result_answer WHERE FORM_ID = 2 AND FIELD_ID = 8 ORDER BY ID DESC LIMIT " . $arParams["LIMIT"]);
    $arAnswer1 = [];
    while ($arAnswer = $res->Fetch()) {
        $arAnswer1[] = $arAnswer['USER_TEXT'];
    }




	$arResult["ANSWER"] = $arAnswer1;

    CModule::IncludeModule('iblock');


    $items_sel =  (new CIBlockElement())->GetList([], ["IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ID"=>$arResult["ANSWER"]], 0, 0, [
        'ID',
        'CODE',
        'PROPERTY_artikul',
        'PROPERTY_brand',
