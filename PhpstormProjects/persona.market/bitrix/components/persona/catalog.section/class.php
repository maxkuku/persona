...
class CatalogSectionComponent extends ElementList
{
	...
	public function getSectIdByCode($code){
		$iBlockID = 4;
		if(strlen($code) > 0){
			$arNavParams = [];
			$arFilter = [ "IBLOCK_ID" => $iBlockID, "CODE" => $code, "ACTIVE" => "Y"];
			$rsSections = CIBlockSection::GetList("", $arFilter, false, array("ID","IBLOCK_ID","IBLOCK_SECTION_ID","NAME","DESCRIPTION","UF_*"),$arNavParams);
			...
			return "Error. No code given to function getSectIdByCode(). File " . __FILE__ . ", line " . __LINE__;
		}
	}

	public function getPageText($id, $code){

		if($id < 1 && strlen($code) > 0){
			$id = $this::getSectIdByCode($code);
		}
		...
		if(!$arResult['ITEMS_SELECTED'])
			$res[0] = "Error. No items selected from iblock 11. File " . __FILE__ . ", line " . __LINE__;
		else
	...
	public function getBrandList($id,$code){
		if($id < 1 && strlen($code) > 0) {
			$id = $this::getSectIdByCode($code);

		}
		if($id > 0){
			$filter = array ( "IBLOCK_ID" => 4, "!=PROPERTY_BRAND" => false, "ACTIVE" => "Y" );
			$items_sel =  (new CIBlockElement())->GetList([], $filter, ["PROPERTY_BRAND"], 0, ['PROPERTY_BRAND_VALUE']);
			$arResult['ITEMS_SELECTED'] = $items_sel->SelectedRowsCount();
			$res = "";
			if(!$arResult['ITEMS_SELECTED'])
				$res = "Error. No items selected from iblock 4. File " . __FILE__ . ", line " . __LINE__;
			else
				while($item = $items_sel->Fetch())
					$res[] = $item;
			...
			return "Error. No code given to function getBrandList(). File " . __FILE__ . ", line " . __LINE__;
		}
	}
}