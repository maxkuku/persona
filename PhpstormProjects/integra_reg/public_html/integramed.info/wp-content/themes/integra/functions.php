...
function get_by_parent($a){ // 991 специалисты
    ...
    foreach( $pages as $post ){
        ...
        ?>
        <div class="card col-12" data-date="<?=$post->post_date?>">
	...
	function twentynineteen_setup() {
		...
		register_nav_menus(
			array(
				...
				'menu-14'       => __( 'Services', 'twentynineteen' ),
				'menu-15'       => __( 'Onco', 'twentynineteen' ),
...
function Services_sidebar() {
    register_sidebar(
        array (
            'name' => __( 'Services', 'integra' ),
            'id' => 'services-side-bar',
            'description' => __( 'Services Sidebar', 'integra' ),
        )
    );
}
add_action( 'widgets_init', 'Services_sidebar' );
...
# add sidebar
function Onco_sidebar() {
    register_sidebar(
        array (
            'name' => __( 'Onco', 'integra' ),
            'id' => 'onco-side-bar',
            'description' => __( 'Onco Sidebar', 'integra' ),
        )
    );
}
add_action( 'widgets_init', 'Onco_sidebar' );
...
function get_half_page($adr){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $adr['link']);
    curl_setopt($ch, CURLOPT_HEADER, 0);
    curl_exec($ch);
    curl_close($ch);
}
add_shortcode('half_page', 'get_half_page');